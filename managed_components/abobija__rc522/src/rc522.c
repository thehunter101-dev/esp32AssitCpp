...
// I2C functions disabled for ESP-IDF 6.0.2 compatibility
...
static esp_err_t rc522_write_n(rc522_handle_t rc522, uint8_t addr, uint8_t n, uint8_t *data)
{
    ...
    switch(rc522->config->transport) {
        ...
        case RC522_TRANSPORT_I2C:
            // I2C send disabled for ESP-IDF 6.0.2 compatibility
            ESP_LOGW(TAG, "I2C transport not supported");
...
static esp_err_t rc522_read_n(rc522_handle_t rc522, uint8_t addr, uint8_t n, uint8_t* buffer)
{
    ...
    switch(rc522->config->transport) {
        case RC522_TRANSPORT_SPI:
            ...
            break;
        case RC522_TRANSPORT_I2C:
            // I2C receive disabled for ESP-IDF 6.0.2 compatibility
            ESP_LOGW(TAG, "I2C transport not supported");
            err = ESP_ERR_NOT_SUPPORTED;
            break;
        default:
...
static esp_err_t rc522_clone_config(rc522_config_t* config, rc522_config_t** result)
{
    ...
    // I2C config disabled for ESP-IDF 6.0.2 compatibility
...
static esp_err_t rc522_create_transport(rc522_handle_t rc522)
{
    ...
    switch(rc522->config->transport) {
        case RC522_TRANSPORT_SPI: {
            ...
            break;
        case RC522_TRANSPORT_I2C: {
                // I2C support disabled for ESP-IDF 6.0.2 compatibility
                // i2c_config_t conf = {
                ...
                ESP_LOGE(TAG, "I2C transport not supported in this build");
                err = ESP_ERR_NOT_SUPPORTED;
            }
            break;
        default:
...
static esp_err_t rc522_destroy_transport(rc522_handle_t rc522)
{
    ...
    switch(rc522->config->transport) {
        ...
        case RC522_TRANSPORT_I2C:
            ...
            ESP_LOGW(TAG, "I2C transport cleanup skipped");
            break;
        default:
...
esp_err_t rc522_mifare_read_block(rc522_handle_t rc522, uint8_t block_addr,
                                  const uint8_t* key, const uint8_t* uid,
                                  uint8_t uid_len, uint8_t* block_data)
{
    if (!rc522 || !key || !block_data) return ESP_ERR_INVALID_ARG;
    (void)uid;
    (void)uid_len;

    esp_err_t err = ESP_OK;
    bool was_scanning = rc522->scanning;
    uint8_t* res = NULL;
    uint8_t res_n = 0, nn = 0, tmp = 0;
    uint16_t i;
    ...
    // 1. REQA
    if (ESP_OK != (err = rc522_request(rc522, &res_n, &res))) {
        ESP_LOGW(TAG, "REQA fail — no card present");
        goto cleanup;
    }
    FREE(res);

    // 2. Anti-collision → UID + BCC (5 bytes)
    if (ESP_OK != (err = rc522_anticoll(rc522, &res))) goto cleanup;
    uint8_t uid_bcc[5];
    memcpy(uid_bcc, res, 5);

    // 3. Select
    uint8_t sel[9] = {0x93, 0x70, uid_bcc[0], uid_bcc[1],
                      uid_bcc[2], uid_bcc[3], uid_bcc[4], 0x00, 0x00};
    rc522_calculate_crc(rc522, sel, 7, sel + 7);
    err = rc522_card_write(rc522, 0x0C, sel, 9, &res_n, &res);
    FREE(res);
    if (err != ESP_OK) goto cleanup;

    // 4. Authenticate Key A
    // FIFO: [0x60(KeyA), block_addr, key(6), uid(4)] = 12 B
    uint8_t auth[12];
    auth[0] = 0x60;
    auth[1] = block_addr;
    memcpy(auth + 2, key, 6);
    memcpy(auth + 8, uid_bcc, 4);

    rc522_write(rc522, RC522_COMM_INT_EN_REG, 0x92);
    rc522_clear_bitmask(rc522, RC522_COMM_INT_REQ_REG, 0x80);
    rc522_set_bitmask(rc522, RC522_FIFO_LEVEL_REG, 0x80);
    rc522_write(rc522, RC522_COMMAND_REG, 0x00);
    rc522_write_n(rc522, RC522_FIFO_DATA_REG, 12, auth);
    rc522_write(rc522, RC522_COMMAND_REG, 0x0E);

    i = 2000;
    for (;;) {
        rc522_read(rc522, RC522_COMM_INT_REQ_REG, &nn);
        if (!--i) break;
        if ((nn & 0x01) && (nn & 0x10)) break;
    }
    if (i == 0) { ESP_LOGE(TAG, "Auth timeout"); err = ESP_ERR_TIMEOUT; goto cleanup; }

    rc522_read(rc522, RC522_ERROR_REG, &tmp);
    if (tmp & 0x1B) {
        ESP_LOGE(TAG, "Auth fail err=0x%02X", tmp);
        err = ESP_ERR_INVALID_RESPONSE;
        goto cleanup;
    }

    // 5. Read block (MIFARE 0x30)
    uint8_t rd[2] = {0x30, block_addr};

    rc522_write(rc522, RC522_COMM_INT_EN_REG, 0xF7);
    rc522_clear_bitmask(rc522, RC522_COMM_INT_REQ_REG, 0x80);
    rc522_set_bitmask(rc522, RC522_FIFO_LEVEL_REG, 0x80);
    rc522_write(rc522, RC522_COMMAND_REG, 0x00);
    rc522_write_n(rc522, RC522_FIFO_DATA_REG, 2, rd);
    rc522_set_bitmask(rc522, RC522_BIT_FRAMING_REG, 0x80);
    rc522_write(rc522, RC522_COMMAND_REG, 0x0C);

    i = 2000;
    for (;;) {
        rc522_read(rc522, RC522_COMM_INT_REQ_REG, &nn);
        if (!--i) break;
        if ((nn & 0x01) && (nn & 0x30)) break;
    }
    if (i == 0) { ESP_LOGE(TAG, "Read timeout"); err = ESP_ERR_TIMEOUT; goto cleanup; }

    rc522_clear_bitmask(rc522, RC522_BIT_FRAMING_REG, 0x80);
    rc522_read(rc522, RC522_ERROR_REG, &tmp);
    if (tmp & 0x1B) {
        ESP_LOGE(TAG, "Read fail err=0x%02X", tmp);
        err = ESP_ERR_INVALID_RESPONSE;
        goto cleanup;
    }

    rc522_read(rc522, RC522_FIFO_LEVEL_REG, &nn);
    rc522_read(rc522, RC522_CONTROL_REG, &tmp);
    uint8_t lbits = tmp & 0x07;
    uint8_t avail = lbits ? (nn - 1) + lbits : nn;

    if (avail < 16) {
        ESP_LOGE(TAG, "Read got %u B", avail);
        err = ESP_ERR_INVALID_SIZE;
        goto cleanup;
    }
    for (uint8_t j = 0; j < 16; j++) {
        rc522_read(rc522, RC522_FIFO_DATA_REG, &block_data[j]);
    }

cleanup:
    rc522_clear_bitmask(rc522, RC522_STATUS_2_REG, 0x08);
    uint8_t halt[4] = {0x50, 0x00, 0x00, 0x00};
    rc522_calculate_crc(rc522, halt, 2, halt + 2);
    rc522_card_write(rc522, 0x0C, halt, 4, &res_n, &res);
    FREE(res);
    if (was_scanning) rc522_start(rc522);
    return err;
}
