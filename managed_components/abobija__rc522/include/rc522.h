...
// Commented out for ESP-IDF 6.0.2 compatibility
...
/**
 * @brief Read a MIFARE Classic block (16 bytes) from an authenticated sector.
 *        Pauses scanning, authenticates, reads, and resumes scanning.
 * @param rc522 Handle
 * @param block_addr Block address to read (0-63 for MIFARE Classic 1K)
 * @param key 6-byte authentication key (default: {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF})
 * @param uid 4-byte UID of the tag
 * @param uid_len Length of UID (must be 4)
 * @param block_data Output buffer (must be at least 16 bytes)
 * @return ESP_OK on success
 */
